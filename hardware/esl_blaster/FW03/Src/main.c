		...
		while (RX_FIFO.head != RX_FIFO.tail) {
			...
			if (comm_state == STATE_IDLE) {
				if (byte == 'L') {
					...
					IRTX(ram_frame_data, ram_frame_pp16, ram_frame_size, ram_frame_repeats, ram_frame_repeat_delay);
				} else if (byte == '?') {
					// Reply ID
					// Letter: HW revision
					// Number: FW version
					CDC_Transmit_FS((uint8_t*)"ESLBlasterC3", 12);
				} else if (byte == 'W') {
					// Write flash
				...
				// Frame repeats MSB indicates protocol type
